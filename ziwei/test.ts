 ...
 */
async function runTestCase(testCase: ChartInput, caseName: string): Promise<BirthChart> {
  console.log(`\n${'='.repeat(60)}`);
  ...
  try {
    const chart = await calculateBirthChart(testCase);
    ...
    console.log(`Life Palace Index: ${chart.mingGongIndex} (${PALACE_ORDER[chart.mingGongIndex]})`);
    console.log(`Body Palace Index: ${chart.shenGongIndex} (${PALACE_ORDER[chart.shenGongIndex]})`);
    ...
    chart.palaces.forEach((palace, index) => {
      ...
      const markers = [];
      if (isMingGong) markers.push('命宫');
      if (isShenGong) markers.push('身宫');
      ...
      const siHuaArr: string[] = [];
      if (palace.siHua.lu) siHuaArr.push(`禄:${palace.siHua.lu}`);
      if (palace.siHua.quan) siHuaArr.push(`权:${palace.siHua.quan}`);
      if (palace.siHua.ke) siHuaArr.push(`科:${palace.siHua.ke}`);
      if (palace.siHua.ji) siHuaArr.push(`忌:${palace.siHua.ji}`);
 ...
 */
function validateChart(chart: BirthChart): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
  ...
  // Check that there are 12 palaces
  if (chart.palaces.length !== 12) {
    errors.push(`Expected 12 palaces, got ${chart.palaces.length}`);
  }
  ...
  if (!allStarNames.includes(chart.siHua.huaLu as string)) {
    warnings.push(`化禄 star ${chart.siHua.huaLu} not found in chart`);
  }
  if (!allStarNames.includes(chart.siHua.huaQuan as string)) {
    warnings.push(`化权 star ${chart.siHua.huaQuan} not found in chart`);
  }
  if (!allStarNames.includes(chart.siHua.huaKe as string)) {
    warnings.push(`化科 star ${chart.siHua.huaKe} not found in chart`);
  }
  if (!allStarNames.includes(chart.siHua.huaJi as string)) {
  ...
  // Check that main stars are distributed (14 main stars distributed across ~7 palaces on average)
  const palacesWithMainStars = chart.palaces.filter(p => p.mainStars.length > 0).length;
  if (palacesWithMainStars < 5) {
    warnings.push(`Only ${palacesWithMainStars} palaces have main stars (expected at least 5)`);
  }
...
async function runAllTests(): Promise<void> {
  ...
  const testCases: Array<{ input: ChartInput; name: string }> = [
  ...
  const results: Array<{ name: string; chart: BirthChart; validation: ReturnType<typeof validateChart> }> = [];
 ...
 */
async function quickTest(): Promise<BirthChart> {
  console.log('\nQuick Test: Calculating chart for 1990-01-15 10:00\n');
  
  const chart = await calculateBirthChart(TEST_CASE_1);
