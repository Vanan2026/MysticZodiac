 ...
 */
const TIANFU_CONSTELLATION: MainStar[] = [
...
/**
 * Ziwei Star Position Table based on Five Element Bureau and birth day
 * This determines where the Purple Emperor (紫微) is placed
 * 
 * Format: bureau -> dayRange -> palaceIndex offset from Life Palace
 */
type BureauRanges = { ranges: Array<{ min: number; max: number; offset: number }> };

const ZIWEI_POSITION_TABLE: Record<string, BureauRanges> = {
  '水二局': { // Water Bureau (水二局)
    ranges: [
 ...
 */
export function getZiweiPosition(bureau: FiveElementBureau, birthDay: number): number {
 ...
 */
export function placeZiweiConstellation(
  ziweiPosition: number,
  mingGongIndex: number
): Map<number, MainStar[]> {
  const result = new Map<number, MainStar[]>();
 ...
 * @returns Map of palace index to main stars in that palace
 */
export function placeTianfuConstellation(
  ziweiPosition: number,
  mingGongIndex: number
): Map<number, MainStar[]> {
  const result = new Map<number, MainStar[]>();
 ...
 */
export function getMingGongMainStar(
  mainStarsMap: Map<number, MainStar[]>,
  mingGongIndex: number
): MainStar | null {
 ...
 */
export function applyMainStarsToPalaces(
  palaces: PalaceData[],
  mainStarsMap: Map<number, MainStar[]>
): PalaceData[] {
 ...
 */
export function getStarPlacementSummary(
  bureau: FiveElementBureau,
  birthDay: number,
  mingGongIndex: number
): {
  ...
  ziweiConstellation: Array<{ star: MainStar; palace: Palace; palaceIndex: number }>;
  tianfuConstellation: Array<{ star: MainStar; palace: Palace; palaceIndex: number }>;
} {
  const ziweiPosition = getZiweiPosition(bureau, birthDay);
  const tianfuPosition = (ziweiPosition + 6) % 12;
  ...
  const ziweiConstellation = ZIWEI_CONSTELLATION.map((star, index) => {
    ...
    return {
      star,
      palace: PALACE_ORDER[palaceIndex],
      palaceIndex,
    };
  });
