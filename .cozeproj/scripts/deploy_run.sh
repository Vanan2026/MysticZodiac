...
cd /workspace/projects/app/dist && nohup python3 -m http.server 9090 --bind 0.0.0.0 > /app/work/logs/bypass/console.log 2>&1 &

# Start backend health server on 9091
nohup node /tmp/health-server.mjs > /app/work/logs/bypass/app.log 2>&1 &
