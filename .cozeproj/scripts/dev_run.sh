...
cd "$PROJECT_ROOT/app/dist" && nohup python3 -m http.server 9090 --bind 0.0.0.0 > /tmp/frontend9090.log 2>&1 &

# Start backend health server on 9091
nohup node "$SCRIPT_DIR/health-server.mjs" > /tmp/health-server.log 2>&1 &
